...
public class PickImageHelper {
    ...
    public static void selectImage(final Activity activity) {
       // Log.e(getPickImageChooserIntent(activity).getType(),"cam");
        activity.startActivityForResult(getPickImageChooserIntent(activity), 9162);
        /*Intent sintent=getPickImageChooserIntent(activity);*/
    }
    ...
    public String getrealPath(final Activity activity){

        Uri outputFileUri = getCaptureImageOutputUri(activity);

        File file = new File(getRealPathFromURI(activity, outputFileUri));
        String path=getRealPathFromURI(activity,outputFileUri);
        return path;
    }
    ...
    private static Uri getCaptureImageOutputUri(Activity activity) {
        ...
        if (getImage != null) {
            outputFileUri = Uri.fromFile(new File(Environment.getExternalStorageDirectory(), timestamp));
        }
